...
def main() -> int:
    ...
    parser.add_argument("--samply-analysis", type=Path)
    ...
    samply = (
        json.loads(args.samply_analysis.read_text())
        if args.samply_analysis is not None
        else {"window": None, "top_cpu_leaves": [], "top_offcpu_leaves": []}
    )
    ...
    targeted = [
        ...
        if any(
            token in row["operation"]
            for token in ("seek", "delete", "upsert", "update-current")
        )
    ]
    trie_operations = [
    ...
    page_batch_tables = sorted(
        {
            ...
            for (name, labels), value in after.items()
            if name == "reth_database_page_batch_attempts_total"
            and value > before.get((name, labels), 0)
        }
    )
    page_batches = []
    for table in page_batch_tables:
        ...
        for (name, labels), value in after.items():
            label_map = dict(labels)
            if (
                name == "reth_database_page_batch_fallback_total"
                and label_map.get("table") == table
                and value > before.get((name, labels), 0)
            ):
                reason = label_map.get("reason", "unknown")
                fallback_reasons[reason] = int(
                    round(
                        delta(
                            before,
                            after,
                            "reth_database_page_batch_fallback_total",
                ...
                "source_pages": int(
                    round(
                        delta(
                            before,
                            after,
                            "reth_database_page_batch_source_pages_total",
                            table_label,
                        )
                    )
                ),
                "destination_pages": int(
                    round(
                        delta(
                            before,
                            after,
                            "reth_database_page_batch_destination_pages_total",
                            table_label,
                        )
                    )
                ),
                "source_bytes": int(
                    round(
                        delta(
                            before,
                            after,
                            "reth_database_page_batch_source_bytes_total",
                            table_label,
                        )
                    )
                ),
                "destination_bytes": int(
                    round(
                        delta(
                            before,
                            after,
                            "reth_database_page_batch_destination_bytes_total",
                            table_label,
                        )
                    )
                ),
    ...
    output = {
        "schema_version": "reth-fpga-detail-analysis/v2",
        ...
        "samply_available": args.samply_analysis is not None,
